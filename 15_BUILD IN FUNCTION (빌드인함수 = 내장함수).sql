-- 15. BUILD IN FUNCTION (빌드인함수 = 내장함수)

-- 1. 문자열 관련 함수

-- ASCII(아스키코드): 아스키코드 값 추출, CHAR(숫자): 아스키 코드로 문자 추출;
SELECT ASCII('A'), CHAR(65);

-- BIT_LENGTH(문자열) : 할당 된 비트 크기 변환, 
SELECT BIT_LENGTH('pie'), 

-- CHAR_LENGTH(문자열) : 문자열의 길이 반환, 
CHAR_LENGTH('pie'),

-- LENGTH(문자열) : 할당 된 byte크기 반환
LENGTH('pie');
SELECT 
	menu_name,
...
-- TRIM: TRIM은 기본적으로 앞뒤 공백을 제거하지만 방향(LEADING(앞), BOTH(양쪽), TRAILING(뒤))이 있으면 
-- 해당 방향에 지정한 문자열을 제거할 수 있다.
SELECT TRIM(' MySQL '), TRIM(BOTH '8' FROM '888MySQL@@@@');
...
-- SELECTSUBSTRING: 시작 위치부터 길이만큼의 문자를 반환(길이를 생략하면 시작 위치부터 끝까지 반환)
SELECT
...
-- SUBTIME(날짜시간, 시간): 날짜 또는 시간을 기준으로 시간을 뺌
SELECT
...
-- TIMEDIFF(날짜1또는 시간1, 날짜1 또는 시간2): 시간1 - 시간2의 시간을 반환
SELECT DATEDIFF('2024-01-26', NOW()), TIMEDIFF('17:07:11', '13:06:10');
